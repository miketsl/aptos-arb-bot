...
impl BotConfig {
    ...
    pub fn save<P: AsRef<Path>>(&self, path: P) -> Result<(), ConfigError> {
        ...
        std::fs::write(path.as_ref(), content).map_err(ConfigError::IoError)?;
    ...
    pub fn validate(&self) -> Result<(), ConfigError> {
        ...
        for dex in &self.dexes {
            if dex.name.is_empty() {
                return Err(ConfigError::ValidationError(
                    "DEX name cannot be empty".to_string(),
                ));
            }

            if dex.module_addr.is_empty() {
                return Err(ConfigError::ValidationError(format!(
                    "Module address for DEX '{}' cannot be empty",
                    dex.name
                )));
            }
            ...
            if dex.fullnode_rpc.is_empty() {
                return Err(ConfigError::ValidationError(format!(
                    "Fullnode RPC for DEX '{}' cannot be empty",
                    dex.name
                )));
            }
        ...
        if self.detector.interval_ms == 0 {
            return Err(ConfigError::ValidationError(
                "Detector interval must be greater than 0".to_string(),
            ));
        }

        if self.detector.min_profit_pct < 0.0 {
            return Err(ConfigError::ValidationError(
                "Minimum profit percentage cannot be negative".to_string(),
            ));
        }
