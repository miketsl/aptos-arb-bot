use crate::data_source::{
    DataSource, DataSourceError, EventMetadata, PoolInitialization, RawEvent, RecordedBatch,
    RecordedPoolState, TimestampedEvent,
};
...
impl FileSource {
    ...
    fn process_pool_initializations(
        &mut self,
        pool_states: Vec<RecordedPoolState>,
        batch_timestamp: SystemTime,
    ) {
    ...
    fn create_timestamped_event(
        &mut self,
        transaction: aptos_indexer_processor_sdk::aptos_protos::transaction::v1::Transaction,
        start_version: u64,
    ) -> Result<Option<TimestampedEvent>, DataSourceError> {
        let blockchain_timestamp = transaction.timestamp.and_then(|ts| {
            UNIX_EPOCH
...
mod tests {
    ...
    async fn test_file_source_with_embedded_pool_states() {
        ...
        assert_eq!(
            pool_init.pool_state.additional_data,
            b"{\"tick_spacing\": 100}"
        );
