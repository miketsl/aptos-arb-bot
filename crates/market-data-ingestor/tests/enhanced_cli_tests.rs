...
fn test_recording_config_yaml_serialization() -> Result<()> {
    ...
    assert_eq!(config.recording.file_rotation.enabled, deserialized.recording.file_rotation.enabled);
    assert_eq!(config.pool_detection.enabled, deserialized.pool_detection.enabled);
    assert_eq!(config.monitoring.stats_interval_seconds, deserialized.monitoring.stats_interval_seconds);
...
fn test_file_rotation_manager_creation() -> Result<()> {
    ...
    let manager = FileRotationManager::new(
...
fn test_timestamp_substitution() -> Result<()> {
    ...
    let resolved = market_data_ingestor::file_rotation::FileRotationManager::resolve_output_path(pattern)?;
...
fn test_pool_filter_config_creation() {
    ...
    let pool_filter = market_data_ingestor::pool_state_manager::PoolFilterConfig {
        ...
        token_blacklist: recording_config.pool_detection.filters.token_blacklist.clone(),
        dex_whitelist: recording_config.pool_detection.filters.dex_whitelist.clone(),
        pool_type_whitelist: recording_config.pool_detection.filters.pool_type_whitelist.clone(),
