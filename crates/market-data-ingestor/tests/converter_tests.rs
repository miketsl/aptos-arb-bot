...
fn create_test_protobuf_file(batches: Vec<RecordedBatch>) -> NamedTempFile {
    ...
    for batch in batches {
        ...
        batch.encode_length_delimited(&mut buf).expect("Failed to encode batch");
...
fn test_protobuf_to_json_conversion() {
    ...
    let output = std::process::Command::new("cargo")
        ...
        .arg("--input").arg(temp_pb_file.path())
        .arg("--output").arg(temp_json_file.path())
        .arg("--pretty")
        .arg("--include-metadata")
        .output()
        .expect("Failed to run mdi-converter");

    assert!(output.status.success(), "Conversion failed: {}", String::from_utf8_lossy(&output.stderr));

    // Verify JSON output exists and is valid
    let json_content = fs::read_to_string(temp_json_file.path())
    ...
    let batches = json_value["batches"].as_array().expect("Batches should be array");
    ...
    let pool_inits = batch["pool_initializations"].as_array().expect("Pool initializations should be array");
    assert_eq!(pool_inits.len(), 1, "Should have exactly one pool initialization");
...
fn test_json_to_protobuf_conversion() {
    ...
    let temp_pb_file = create_test_protobuf_file(vec![test_batch.clone()]);
    let temp_json_file = NamedTempFile::new().expect("Failed to create temp JSON file");
    let temp_reconverted_pb_file = NamedTempFile::new().expect("Failed to create temp reconverted PB file");
    ...
    let output = std::process::Command::new("cargo")
        ...
        .arg("--input").arg(temp_json_file.path())
        .arg("--output").arg(temp_reconverted_pb_file.path())
        .arg("--validate")
        .output()
        .expect("Failed to run mdi-converter to-protobuf");

    assert!(output.status.success(), "Protobuf conversion failed: {}", String::from_utf8_lossy(&output.stderr));

    // Verify the reconverted protobuf can be read
    let reconverted_data = fs::read(temp_reconverted_pb_file.path())
        .expect("Failed to read reconverted protobuf");
    ...
    assert_eq!(reconverted_batch.pool_initializations.len(), test_batch.pool_initializations.len());
...
fn test_round_trip_validation() {
    ...
    let output = std::process::Command::new("cargo")
        ...
        .arg("--protobuf").arg(temp_pb_file.path())
        .arg("--json").arg(temp_json_file.path())
        .output()
        .expect("Failed to run mdi-converter validate");

    assert!(output.status.success(), "Round-trip validation failed: {}", String::from_utf8_lossy(&output.stderr));
    
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("Round-trip validation successful"), "Validation should succeed");
    assert!(stdout.contains("Data integrity: PASSED"), "Data integrity should pass");
}
...
fn test_multi_token_pool_conversion() {
    ...
    let multi_token_pool = RecordedPoolState {
        ...
        reserve_b: "500000".to_string(),
        fee_rate: "0.003".to_string(),
        all_tokens: vec!["APT".to_string(), "USDC".to_string(), "USDT".to_string(), "BTC".to_string()],
        all_reserves: vec!["1000000".to_string(), "500000".to_string(), "750000".to_string(), "100".to_string()],
    ...
    let all_tokens = pool["all_tokens"].as_array().expect("all_tokens should be array");
    assert_eq!(all_tokens.len(), 4, "Should have 4 tokens");
    assert_eq!(all_tokens[0], "APT");
    assert_eq!(all_tokens[3], "BTC");
    
    let all_reserves = pool["all_reserves"].as_array().expect("all_reserves should be array");
    assert_eq!(all_reserves.len(), 4, "Should have 4 reserves");
    
    let all_weights = pool["all_weights"].as_array().expect("all_weights should be array");
