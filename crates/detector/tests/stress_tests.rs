...
async fn stress_test_massive_updates() {
    ...
    for block in 1..=num_blocks {
        ...
        tx.send(DetectorMessage::MarketUpdate(
            create_market_update("DEX1", "APT", "USDC", base_price, 1_000_000)
        )).await.unwrap();
        tx.send(DetectorMessage::MarketUpdate(
            create_market_update("DEX1", "USDC", "APT", base_price, 1_000_000)
        )).await.unwrap();
        
        // APT/USDC on DEX2 with price 2x (clear arbitrage opportunity)
        let high_price = base_price * 2;
        tx.send(DetectorMessage::MarketUpdate(
            create_market_update("DEX2", "APT", "USDC", high_price, 1_000_000)
        )).await.unwrap();
        tx.send(DetectorMessage::MarketUpdate(
            create_market_update("DEX2", "USDC", "APT", high_price, 1_000_000)
        )).await.unwrap();
        
        // ETH/USDC on DEX1 with price 1x
        tx.send(DetectorMessage::MarketUpdate(
            create_market_update("DEX1", "ETH", "USDC", base_price, 1_000_000)
        )).await.unwrap();
        tx.send(DetectorMessage::MarketUpdate(
            create_market_update("DEX1", "USDC", "ETH", base_price, 1_000_000)
        )).await.unwrap();
        
        // ETH/USDC on DEX2 with price 3x (another arbitrage opportunity)
        let very_high_price = base_price * 3;
        tx.send(DetectorMessage::MarketUpdate(
            create_market_update("DEX2", "ETH", "USDC", very_high_price, 1_000_000)
        )).await.unwrap();
        tx.send(DetectorMessage::MarketUpdate(
            create_market_update("DEX2", "USDC", "ETH", very_high_price, 1_000_000)
        )).await.unwrap();
    ...
    assert!(test_result.is_ok(), "Massive updates stress test should complete within timeout");
}
...
async fn stress_test_rapid_blocks() {
    ...
    for block in 1..=10 {
        ...
        tx.send(DetectorMessage::MarketUpdate(update_forward)).await.unwrap();
        
        let update_reverse = create_market_update("DEX1", "USDC", "APT", 1u128 << 64, 1_000_000);
        tx.send(DetectorMessage::MarketUpdate(update_reverse)).await.unwrap();
...
async fn stress_test_extreme_values() {
    ...
    tx.send(DetectorMessage::MarketUpdate(
        create_market_update("DEX1", "APT", "USDC", high_price, 1_000_000)
    )).await.unwrap();
    tx.send(DetectorMessage::MarketUpdate(
        create_market_update("DEX1", "USDC", "APT", high_price, 1_000_000)
    )).await.unwrap();
    
    // Very low price (but not zero) - DEX2
    let low_price = (1u128 << 64) / 1_000_000;
    tx.send(DetectorMessage::MarketUpdate(
        create_market_update("DEX2", "APT", "USDC", low_price, 1_000_000)
    )).await.unwrap();
    tx.send(DetectorMessage::MarketUpdate(
        create_market_update("DEX2", "USDC", "APT", low_price, 1_000_000)
    )).await.unwrap();
    
    // Very high liquidity (but reasonable) - DEX3
    tx.send(DetectorMessage::MarketUpdate(
        create_market_update("DEX3", "ETH", "USDC", 1u128 << 64, 1_000_000_000_000u128)
    )).await.unwrap();
    tx.send(DetectorMessage::MarketUpdate(
        create_market_update("DEX3", "USDC", "ETH", 1u128 << 64, 1_000_000_000_000u128)
    )).await.unwrap();
    
    // Very low liquidity (but not zero) - DEX4
    tx.send(DetectorMessage::MarketUpdate(
        create_market_update("DEX4", "ETH", "USDC", 1u128 << 64, 1)
    )).await.unwrap();
    tx.send(DetectorMessage::MarketUpdate(
        create_market_update("DEX4", "USDC", "ETH", 1u128 << 64, 1)
    )).await.unwrap();
    
    // Maximum fee - DEX5 (bidirectional)
    let high_fee_update = MarketUpdate {
    ...
    tx.send(DetectorMessage::MarketUpdate(high_fee_update)).await.unwrap();
    
    let high_fee_update_reverse = MarketUpdate {
    ...
    tx.send(DetectorMessage::MarketUpdate(high_fee_update_reverse)).await.unwrap();
    ...
    assert!(!handle.is_finished(), "Service should handle extreme values without crashing");

    drop(tx);
    handle.await.unwrap();
    
    let opportunities_count = drain_handle.await.unwrap();
    println!("Extreme values test - opportunities detected: {}", opportunities_count);
}
...
async fn stress_test_channel_saturation() {
    ...
    let send_handle = tokio::spawn(async move {
        for block in 1..=20 { // Reduced from 50 to 20 for faster test
            // This might block due to small channel size
            if tx.send(DetectorMessage::BlockStart {
                block_number: block,
                timestamp: Utc::now(),
            }).await.is_err() {
                break;
            ...
            for i in 0..3 { // Reduced from 5 to 3
                ...
                if tx.send(DetectorMessage::MarketUpdate(update)).await.is_err() {
                    break;
                }
            }

            if tx.send(DetectorMessage::BlockEnd { block_number: block }).await.is_err() {
                break;
            }
        }
        tx
    ...
    for _ in 0..15 { // Reduced from 20 to 15
    ...
    println!("  Note: With non-blocking sends, some opportunities may be dropped when channel is full");
...
async fn stress_test_large_tick_maps() {
    ...
    for i in 0..2 {
        ...
        for tick in -10i32..10i32 {
            tick_map.insert(tick, TickInfo {
                ...
                liquidity_gross: (tick.abs() as u128) * 1000 + 1000,
            });
        }
    ...
    assert!(!handle.is_finished(), "Service should handle large tick maps without crashing");

    drop(tx);
    handle.await.unwrap();
    
    let opportunities_count = drain_handle.await.unwrap();
    println!("Large tick maps test - opportunities detected: {}", opportunities_count);
    }).await;
    
    assert!(test_result.is_ok(), "Large tick maps stress test should complete within timeout");
}
...
async fn stress_test_disconnected_components() {
    ...
    // Send all updates
    for update in apt_updates.into_iter().chain(btc_updates).chain(isolated_updates) {
        tx.send(DetectorMessage::MarketUpdate(update)).await.unwrap();
    }
    ...
    assert!(!handle.is_finished(), "Service should handle disconnected components gracefully");

    drop(tx);
    handle.await.unwrap();
    
    let opportunities_count = drain_handle.await.unwrap();
    println!("Disconnected components test - opportunities detected: {}", opportunities_count);
}
...
async fn stress_test_strategy_load() {
            ...
            for (j, &token_b) in tokens.iter().enumerate() {
                if i != j {
                    for &dex in &dexes {
                        ...
                        let price_factor = if (i + j + block as usize) % 2 == 0 { 2 } else { 1 };
                        let sqrt_price = (1u128 << 64) * price_factor;
                        
                        let update = create_market_update(dex, token_a, token_b, sqrt_price, 1_000_000);
                        tx.send(DetectorMessage::MarketUpdate(update)).await.unwrap();
                    }
                }
            }
    ...
    assert!(!handle.is_finished(), "Service should handle multiple strategies without issues");

    drop(tx);
    handle.await.unwrap();
    }).await;
    
    assert!(test_result.is_ok(), "Strategy load stress test should complete within timeout");
}