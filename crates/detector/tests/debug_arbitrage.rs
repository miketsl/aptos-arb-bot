...
async fn debug_simple_arbitrage() {
    ...
    let drain_handle = tokio::spawn(async move {
        ...
        while let Some(opp) = op_rx.recv().await {
            count += 1;
            println!("Opportunity {}: strategy={}, profit={}, input={}", 
    ...
    tx.send(DetectorMessage::MarketUpdate(update1_rev)).await.unwrap();
    ...
    tx.send(DetectorMessage::MarketUpdate(update2_rev)).await.unwrap();
    ...
    assert!(opportunities_count > 0, "Should detect arbitrage opportunity between DEX1 and DEX2");
}