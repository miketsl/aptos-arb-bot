...
async fn benchmark_single_strategy_latency() {
    ...
    for block in 1..=100 {
        ...
        if timeout(Duration::from_millis(50), op_rx.recv())
            .await
            .is_ok()
        {
    ...
    if !latencies.is_empty() {
        ...
        assert!(
            ...
            "Average latency should be < 10ms, got {:?}",
            avg
        );
        assert!(
            p95 < Duration::from_millis(20),
            "P95 latency should be < 20ms, got {:?}",
            p95
        );
    }
...
async fn benchmark_multi_strategy_performance() {
    ...
    if !latencies.is_empty() {
        ...
        println!(
            "  Avg opportunities per block: {:.2}",
            total_opportunities as f64 / latencies.len() as f64
        );

        // Multi-strategy should be reasonable (more complex than single strategy)
        assert!(
            avg_time < Duration::from_millis(200),
            "Multi-strategy average should be < 200ms, got {:?}",
            avg_time
        );
    }
...
async fn benchmark_update_throughput() {
    ...
    assert!(
        throughput > 1000.0,
        "Throughput should be > 1000 updates/sec, got {:.0}",
        throughput
    );
...
async fn benchmark_memory_usage() {
    ...
    for dex in 0..num_dexes {
        for i in 0..num_tokens {
            for j in (i + 1)..std::cmp::min(i + 5, num_tokens) {
                // Each token connects to 4 others
                let update = create_market_update(
                    &format!("DEX{}", dex),
                    &format!("TOKEN{}", i),
                    &format!("TOKEN{}", j),
    ...
    println!(
        ...
        update_count as f64 / processing_time.as_secs_f64()
    );

    // Should handle large graphs efficiently
    assert!(
        processing_time < Duration::from_secs(5),
        "Large graph processing should be < 5s, got {:?}",
        processing_time
    );
